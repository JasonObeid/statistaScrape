...
titlePaths = os.listdir('./unlabelled/titles_old/')
titlePaths.sort()

for summaryPath in titlePaths:
        with open('./unlabelled/titles_old/'+summaryPath, 'r', encoding='utf-8') as titleFile:
                ...
                if lastTokens[1] == '-' and lastTokens[0].isnumeric() and lastTokens[2].isnumeric():
                        cleanTitle[-2] = 'to'
                newTitlePath = './unlabelled/titles/' + summaryPath
                with open(newTitlePath, "w") as outf:
                        # remove asterisks
                        cleanTitle = ' '.join(cleanTitle).replace('*', '')
                        outf.write("{}".format(cleanTitle))
                outf.close()
